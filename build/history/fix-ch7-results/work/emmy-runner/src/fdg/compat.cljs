(ns fdg.compat
  ...
  (:require [emmy.calculus.form-field :as ff]
            ...
            [fdg.slow-checks :as slow-checks]))
...
(def spherical->stereographic slow-checks/spherical->stereographic)

(def transform-stereographic-Christoffel-to-spherical
  slow-checks/transform-stereographic-Christoffel-to-spherical)

(def time first)

(def coordinate second)

(def coordinates second)

(def velocity #(nth % 2))

(def velocities #(nth % 2))

(def d:dt "Canonical time-coordinate vector field used across chapters." (e/coordinate-system->vector-basis e/R1-rect))

(def d:dtau
  "Curve-parameter vector field, kept distinct from spacetime d:dt bindings."
  (e/coordinate-system->vector-basis e/R1-rect))

(defn pair? [value] (boolean (and (seq? value) (seq value))))

(defn series:for-each [f series n] (doseq [term (take n series)] (f term)))

(defn for-each [f values] (doseq [value values] (f value)))

(defn sigma [f low high] (reduce e/+ 0 (map f (range low (inc high)))))

(defn list-with-substituted-coord [values i value] (assoc (vec values) i value))

(defn show-expression [value] value)

(defn- all-zero?
  [value]
  (cond (or (e/structure? value) (sequential? value)) (every? all-zero? value)
        (number? value) (zero? value)
        :else (e/zero? value)))
...
(defn verified-zero
  ...
  (if (all-zero? checks)
    result
    (throw (js/Error. (str "Expected exact zero checks, got " (pr-str (e/freeze checks)))))))

(defn polynomially-verified-zero
  "Prove an expression is exactly zero without rational-function GCD reduction."
  [expression]
  (verified-zero 0 (simplify/*poly-simplify* expression)))
    ...
    (mapv (fn [i]
            (let [thing (literal-field coordsys (nth types i))]
              (-> ((e/- (apply tensor (assoc args i (e/+ (e/* f (get args i)) thing)))
                        (e/+ (e/* f (apply tensor args)) (apply tensor (assoc args i thing))))
...
(defn patch [patch-name manifold-value] (manifold/get-patch manifold-value (emmy-keyword patch-name)))

(defn coordinate-system
  [system-name patch-value]
  (manifold/coordinate-system-at (:manifold patch-value) (emmy-keyword system-name) (:name patch-value)))
...
(def general-Cartan-2 (e/Christoffel->Cartan (e/literal-Christoffel-2 'Gamma e/R2-rect)))
...
(def b-rect ((e/coordinate-system->oneform-basis e/R2-rect) (e/literal-vector-field 'b e/R2-rect)))
...
(def sphere-Cartan (e/Christoffel->Cartan S2-Christoffel))
...
(def home "Base frame assumed, but not defined, by the Chapter 11 text." (e/base-frame-maker 'home 'home))
...
(defn legacy-frame-maker
  ...
  (let [make (e/frame-maker
               (fn [ancestor-frame this-frame params] (apply coordinates->event ancestor-frame this-frame params))
               (fn [ancestor-frame this-frame params] (apply event->coordinates ancestor-frame this-frame params)))]
    (fn [name ancestor-frame & params] (make name ancestor-frame params))))
...
(defn show "Identity helper useful while inspecting intermediate values in SCI." [value] value)
