...
(let [x (literal-function 'x)
  ...
  if (id === "chapter009-020") {
    // Here x, y and z are manifold coordinate functions. scmutils constructs
  ...
  if (id === "chapter008-017") {
    ...
    // coordinate, but no top-level coordinate binding exists in this section.
    if (source.startsWith("(let [t 't]")) return source;
    return `(let [t 't]\n${source.split("\n").map(line => `  ${line}`).join("\n")})`;
  }
