...
const nonExecutableIds = new Set([
  // This is a displayed symbolic coefficient structure containing an unbound
  // coordinate symbol, not a standalone calculation from the preceding block.
  "chapter008-017",
  // This is the compact expected leading term of the preceding Ricci
  // calculation; Emmy cannot evaluate its literal-function derivatives as a
  // standalone expression ("Function input not yet supported").
  "chapter009-020",
]);

// These are valid interactive examples, but either print a lazy series or are
// too expensive for the deterministic every-build smoke pass.
const smokeExcludedIds = new Set([
  "chapter003-010", "chapter007-011",
...
const prerequisiteIds = new Map([
  ...
  // The source presents this shared setup in a footnote whose extracted block
  // lands at the end of the chapter. Run it before the examples that use it.
  ["chapter007-006", ["chapter007-043"]],
  ["chapter007-007", ["chapter007-043"]],
  ["chapter007-008", ["chapter007-043"]],
  ["chapter007-009", ["chapter007-043"]],
]);
...
for (const file of files) {
  ...
  blocks.forEach((block, index) => {
    ...
    const simplified = boundSlowSimplifications(
      positionSimplifyExplanation(
