...
import { cpSync, existsSync, lstatSync, mkdirSync, readFileSync, readdirSync, renameSync, rmSync, statSync, symlinkSync, writeFileSync } from "node:fs";
...
function promoteCurrent() {
  ...
  symlinkSync(path.join("history", runId), currentDir, "dir");

  const retained = readdirSync(historyDir)
    .map(name => ({ name, path: path.join(historyDir, name), time: statSync(path.join(historyDir, name)).mtimeMs }))
    .sort((a, b) => b.time - a.time);
  for (const stale of retained.slice(3)) rmSync(stale.path, { recursive: true, force: true });
  return finalDir;
}
...
try {
  ...
  finish("succeeded", { workspace: path.join(buildDir, "current", "work") });
  ...
  promoteCurrent();
  console.log(`FDG build artifacts: ${path.join(buildDir, "current", "work")}`);
} catch (error) {
