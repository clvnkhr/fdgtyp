...
const appendixIIds = [...contentByFile.get("appendix_i.typ").matchAll(/\b(I-\d{3})\b/g)]
  .map(match => match[1]);
...
for (const file of contentFiles) {
  ...
  if (file === "preface_cljs.typ") {
    if (!text.startsWith("// Generated by ../scripts/convert-org-to-typst.mjs.")) {
      fail("ClojureScript preface stub is not generated by the converter");
    }
    if (!text.includes('#include "../fdg-lib/preface_cljs.typ"')) {
      fail("ClojureScript preface stub must include the authored fdg-lib document");
    }
    if (matchAll(/#fdg-chapter\(/g, text).length !== 0) {
      fail("ClojureScript preface content belongs in fdg-lib, not the generated stub");
    }
    continue;
  }
