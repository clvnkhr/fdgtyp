...
#align(center)[#image("../assets/figures/fig-6-1.pdf", width: 92%)]
...
The internal procedure make-fake-vector-field counterfeits a vector field $sans(u)$ on $sans(M)$ from the vector field over the map $mu : sans(N) arrow.r sans(M)$. This works here because the only value that is ever passed as `m` is `(mu:N->M n)`.
