...
The mathematical notation is in one-to-one correspondence with expressions of ClojureScript with the Emmy computer algebra library. ClojureScript is a Lisp and is based on the $lambda$-calculus
...
$ d (x_1\,y_1\,x_2\,y_2)= sqrt((x_2 - x_1)^2+(y_2 - y_1)^2) . $ <E.1>
...
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/001")
...
;; .7518269446689928
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/002")
...
;; .7518269446689928
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/003")
...
;; 7.274379414605454
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/004")

== Symbolic Values <sec-E.2>
As in usual mathematical notation, arithmetic is extended to allow the use of symbols that represent unknown or incompletely specified mathematical objects. These symbols are manipulated as if they had values of a known type. By default, a ClojureScript symbol is assumed to represent a real number. So the expression #raw(lang:"clojure", "'a") is a literal ClojureScript symbol that represents an unspecified real number:
...
;; (f (g x))
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/007")

We can also make literal functions of multiple, possibly structured arguments that return structured values. For example, to denote a literal function named #raw(lang:"clojure", "g") that takes two real arguments and returns a real value ($g : upright(bold(R)) times upright(bold(R)) arrow.r upright(bold(R))$) we may write:
...
;; (g x y)
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/008")

We may use such a literal function anywhere that an explicit function of the same type may be used.

There is a whole language for describing the type of a literal function in terms of the number of arguments, the types of the arguments, and the types of the values. Here we describe a function that maps pairs of real numbers to real numbers with the expression #raw(lang:"clojure", "(-> (X Real Real) Real)"). Later we will introduce structured arguments and values and show extensions of literal functions to handle these.
...
We make tuples with the constructors #raw(lang:"clojure", "up") and #raw(lang:"clojure", "down"):
...
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/010")
...
;; b
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/013")
...
$ p v = p_0 v^0 + p_1 v^1 + p_2 v^2 . $ <E.8>

So the product of tuples that are compatible for contraction is an inner product. Using the tuples #raw(lang:"clojure", "p") and #raw(lang:"clojure", "v") defined above gives us

/* fdg-code-source: appendix_b/014
(* p v)
;; (+ (* p 0 vˆ0) (* p 1 vˆ1) (* p 2 vˆ2))
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/014")
...
$ mat(delim: "[", cos theta - sin theta, sin theta cos theta) $ <E.11>

can be represented as a tuple structure:#footnote[To emphasize the relationship of simple tuple structures to matrix notation we often format #raw(lang:"clojure", "up") tuples as vertical arrangements of components and #raw(lang:"clojure", "down") tuples as horizontal arrangements of components. However, we could just as well have written this tuple as $[(cos theta \, sin theta) \, (- sin theta \, cos theta)]$.]

$ [vec(cos theta, sin theta) vec(- sin theta, cos theta)] . $ <E.12>

Such a tuple is compatible for contraction with an up tuple that represents a vector. So, for example:

$ [vec(cos theta, sin theta) vec(- sin theta, cos theta)] vec(x, y) = vec(x cos theta - y sin theta, x sin theta + y cos theta) . $ <E.13>
...
$ [vec(cos theta, sin theta) vec(- sin theta, cos theta)] [vec(cos phi, sin phi) vec(- sin phi, cos phi)]  &= [vec(cos(theta + phi), sin(theta + phi)) vec(- sin(theta + phi), cos(theta + phi))] . $ <E.14>
...
;; (cos x)
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/015")
...
$ f (x + Delta x)approx f (x)+ D f (x)Delta x . $ <E.15>
...
$ f (x + Delta x)=(x + Delta x)^3= x^3 + 3 x^2 Delta x + 3 x Delta x^2 + Delta x^3 $ <E.16>

and

$ D f (x)Delta x = 3 x^2 Delta x . $ <E.17>
...
$ D (f compose g)= ((D f) compose g) dot.op D g . $ <E.18>

So at $x$,

$ (D (f compose g)) (x)= D f (g (x))dot.op D g (x). $ <E.19>

#raw(lang:"clojure", "D") is an example of an operator. An operator is like a function except that multiplication of operators is interpreted as composition, whereas multiplication of functions is multiplication of the values (see equation @E.3). If $D$ were an ordinary function, then the rule for multiplication would imply that $D^2 f$ would just be the product of $D f$ with itself, which is not what is intended. A product of a number and an operator scales the operator. So, for example

/* fdg-code-source: appendix_b/016
(((* 5 D) cos) 'x)
;; (* -5 (sin x))
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/016")
...
;; (+ (((expt D 2) f) x) (* -1 (f x)))
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/017")

== Derivatives of Functions of Multiple Arguments <sec-E.5>
...
$ D g (x\,y)dot.op (Delta x\,Delta y)= [partial_0 g (x \, y) + partial_1 g (x \, y)] dot.op (Delta x\,Delta y) = partial_0 g (x\,y)Delta x + partial_1 g (x\,y)Delta y . $ <E.20>

Using the two-argument literal function #raw(lang:"clojure", "g") defined in Section #fdg-ref-page(<sec-E.2>), we have:

/* fdg-code-source: appendix_b/018
((D g) 'x 'y)
;; (down (((partial 0) g) x y) (((partial 1) g) x y))
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/018")

In general, partial derivatives are just the components of the derivative of a function that takes multiple arguments (or structured arguments or both; see below). So a partial derivative of a function is a composition of a component selector and the derivative of that function.#footnote[Partial derivative operators such as #raw(lang:"clojure", "(partial 2)") are operators, so #raw(lang:"clojure", "(expt (partial 1) 2)") is a second partial derivative.] Indeed:

$ partial_0 g = I_0 compose D g\, $ <E.21>

$ partial_1 g = I_1 compose D g . $ <E.22>
...
$ D g (x\,y)=[3 x^2 y^5\,5 x^3 y^4] $ <E.24>

and the first-order approximation of the increment for changing the arguments by $Delta x$ and $Delta y$ is

$ g (x + Delta x\,y + Delta y)- g (x\,y)approx[3 x^2 y^5\,5 x^3 y^4]dot.op (Delta x\,Delta y) &= 3 x^2 y^5 Delta x + 5 x^3 y^4 Delta y . $ <E.25>

Partial derivatives of compositions also obey a chain rule:

$ partial_i (f compose g)=((D f)compose g)dot.op partial_i g . $ <E.26>
...
(down (((partial 0) g) x y) (((partial 1) g) x y))
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/019")

A phase-space state function is a function of time, coordinates, and momenta. Let $H$ be such a function. The value of $H$ is $H (t \, (x \, y) \, [p_x \, p_y])$ for time $t$, coordinates $(x\,y)$, and momenta $[p_x\,p_y]$. Let $s$ be the phase-space state tuple as in #ref(<E.6>):

$ s = (t \, (x \, y) \, [p_x \, p_y]) . $ <E.28>
...
$ D H (s) &= [partial_0 H (s) \, partial_1 H (s) \, partial_2 H (s)] \
 ...
 [partial_(2\,0) H (s) \, partial_(2\,1) H (s)]]\, $ <E.29>
...
$ Delta s = (Delta t \, (Delta x \, Delta y) \, [Delta p_x \, Delta p_y]) $ <E.30>

then

$ D H (s)Delta s = partial_0 H (s)Delta t + partial_(1\,0) H (s)Delta x + partial_(1\,1) H (s)Delta y + partial_(2\,0) H (s)Delta p_x + partial_(2\,1) H (s)Delta p_y . $ <E.31>
...
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/020")

== Structured Results <sec-E.6>
...
(define (helix t)
  (up (cos t) (sin t) t))
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/021")
...
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/022")
...
(up (* -1 (sin t)) (cos t) 1)
fdg-code-source-end */
#fdg-cljs-code-block("appendix_b/023")
...
$ g (x\,y)= ((x + y)^2 \, (y - x)^3 \, e^(x + y))\, $ <E.34>

then the derivative of $g$ is

$ D g (x\,y)= [vec(2(x + y), - 3(y - x)^2, e^(x + y)) \, vec(2(x + y), 3(y - x)^2, e^(x + y))] $ <E.35>
