...
#fdg-chapter("Vector Fields and One-Form Fields", numbered: true)[
...
$ D g (y)Delta y = D f (phi.alt (y)) (D phi.alt (y)Delta y)= D f (x)Delta x . $
...
The vector field $sans(v)$ has a coordinate representation $v$: $ sans(() sans(f)) (sans(m))= D (f compose chi^(-1)) (chi (sans(m)))b (chi (sans(m)))= D f (x)b (x)= v (f) (x)\, $ with the definitions $f = sans(f) compose chi^(-1)$ and $x = chi (sans(m))$. The function $b$ is the coefficient function for the vector field $sans(v)$. It provides a scale factor for the component in each coordinate direction. However, $v$ is the coordinate representation of the vector field $sans(v)$ in that it takes directional derivatives of coordinate representations of manifold functions.
...
Vector fields have the following properties. Let $sans(u)$ and $sans(v)$ be vector fields and let $alpha$ be a real-valued manifold function. Then $ (sans(u) + sans(v)) (f)= sans(u) (sans(f))+ sans(v) (sans(f)) (alpha sans(f)) (sans(f))= alpha (sans(u) (sans(f))). $

Vector fields are linear operators. Assume $sans(f)$ and $sans(g)$ are functions on the manifold, $a$ and $b$ are real constants.#footnote[If $sans(f)$ has structured output then $sans(v) (sans(f))$ is the structure resulting from $sans(v)$ being applied to each component of $sans(f)$.] The constants $a$ and $b$ are not manifold functions, because vector fields take derivatives. See equation (3.13). $ sans(v) (a sans(f) + b sans(g)) (sans(m))= a sans(v) (sans(f)) (sans(m))+ b sans(v) (sans(g)) (sans(m))sans(v) (a sans(f)) (sans(m))= a sans(v) (sans(f)) (sans(m)) $ Vector fields satisfy the product rule (Leibniz rule). $ sans(() sans(f g)) (sans(m))= sans(v) (sans(f)) (sans(m))sans(g) (sans(m))+ sans(f) (sans(m))sans(v) (sans(g)) (sans(m)) $ Vector fields satisfy the chain rule. Let $F$ be a function on the range of $sans(f)$. $ sans(v) (F compose sans(f)) (sans(m))= D F (sans(f) (sans(m)))sans(v) (sans(f)) (sans(m)) $

== Coordinate-Basis Vector Fields
For an $n$-dimensional manifold any set of $n$ linearly independent vector fields#footnote[A set of vector fields, ${ sans(v)_i }$, is linearly independent with respect to manifold functions if we cannot find nonzero manifold functions, ${ sans(a)_i }$, such that $ sum_i sans(a)_i sans(v)_i(sans(f))= sans(0) (sans(f))\, $ where $sans(0)$ is the vector field such that $sans(0) (sans(f)) (sans(m))= 0$ for all $sans(f)$ and $sans(m)$.] form a #emph[basis] in that any vector field can be expressed as a linear combination of the basis fields with manifold-function coefficients. Given a coordinate system we can construct a basis as follows: we choose the component tuple $b_i (x)$ (see equation 3.5) to be the $i$th unit tuple $u_(() x)$---an up tuple with one in the $i$th position and zeros in all other positions---selecting the partial derivative in that direction. Here $u_i$ is a constant function. Like $b$, it formally takes coordinates of a point as an argument, but it ignores them. We then define the basis vector field $sans(X)_i$ by $ sans(X)_i(sans(f)) (sans(m))= D (sans(f) compose chi^(-1)) (chi (sans(m)))u_i (chi (sans(m)))= partial_i (sans(f) compose chi^(-1)) (chi (sans(m))). $

In terms of $sans(X)_i$ the vector field of equation (3.6) is $ sans(v) (sans(f)) (sans(m))= sum_i sans(X)_i(sans(f)) (sans(m))b^i (chi (sans(m))). $ We can also write $ sans(v) (sans(f)) (sans(m))= sans(X) (sans(f)) (sans(m))b (chi (sans(m)))\, $ letting the tuple algebra do its job.

The basis vector field is often written $ frac(partial, partial x^i) = sans(X)_i\, $ to call to mind that it is an operator that computes the directional derivative in the ith coordinate direction.
...
Consider a coordinate change from the chart $chi$ to the chart $chi'$. $ sans(sans(X)) (sans(f)) (m)= D (sans(f) compose chi^(-1)) (chi (sans(m)))= D (sans(f) compose (chi')^(-1)compose chi' compose chi^(-1)) (chi (sans(m)))= D (sans(f) compose (chi')^(-1)) (chi'(sans(m))) (D (chi' compose chi^(-1))) (chi (sans(m)))= sans(X)'(sans(f)) (sans(m)) (D (chi' compose chi^(-1))) (chi (sans(m))). $ This is the rule for the transformation of basis vector fields. The second factor can be recognized as \`\`∂x\'/∂x,\'\' the Jacobian.#footnote[This notation helps one remember the transformation rule:
...
The vector field does not depend on coordinates. So, from equation (3.17), we have $ sans(v) (sans(f)) (sans(m))= sans(X) (sans(f)) (sans(m))b (chi (sans(m)))= sans(X)'(sans(f)) (sans(m))b'(chi'(sans(m))). $ Using equation (3.19) with $x = chi (sans(m))$ and $x' = chi'(sans(m))$, we deduce $ D (chi' compose chi^(-1)) (x)b (x)= b'(x'). $ Because $chi' compose chi^(-1)$ is the inverse function of $chi compose (chi')^(-1)$, their derivatives are multiplicative inverses, $ D (chi' compose chi^(-1)) (x)=(D (chi compose (chi')^(-1)) (x'))^(-1)\, $ and so $ b (x)= D (chi compose (chi')^(-1)) (x')b'(x')\, $ as expected.#footnote[For coordinate paths $q$ and $q'$ related by $q (t)=(chi compose (chi')^(-1)) (q'(t))$ the velocities are related by $D q (t)= D (chi compose (chi')^(-1)) (q'(t))D q'(t)$. Abstracting off paths, we get $v = D (chi compose (chi')^(-1)) (x')v'$.]
...
Iterating the process described by equation (3.24) we can compute higher-order derivatives of functions along the integral curve: $ D (sans(f) compose gamma)= sans(v) (sans(f))compose gamma D^2 (sans(f) compose gamma)= D (sans(v) (sans(f))compose gamma)= sans(v) (sans(v) (sans(f)))compose gamma dots.c D^n (sans(f) compose gamma)= sans(v)^n (sans(f))compose gamma $

Thus, the evolution of $sans(f) compose gamma$ can be written formally as a Taylor series in the parameter: $ (f compose gamma) (t)=(f compose gamma) (0)+ t D (sans(f) compose gamma) (0)+ 1 / 2 t^2 D^2 (sans(f) compose gamma) (0)+ dots.c =(e^(t D) (sans(f) compose gamma)) (0)= e^(t sans(v) sans(f)) (gamma (0)). $ Using $phi.alt$ rather than $gamma$ $ (sans(f) compose gamma_(sans(m))^(sans(v))) (t)=(sans(f) compose phi.alt_t^(sans(v))) (sans(m))\, $ so, when the series converges, $ (e^(t sans(v)) sans(f)) (sans(m))=(sans(f) compose phi.alt_t^(sans(v))) (sans(m)). $

In particular, let $sans(f) = chi$, then $ sigma (t)=(chi compose gamma) (t)=(e^(t D) (chi compose gamma)) (0)=(e^(t sans(v)) chi) (gamma (0))\, $ a Taylor series representation of the solution to the differential equation (3.27).
...
We can define an evolution operator $sans(E)_(Delta t\,sans(v))$ using equation (3.31) $ (E_(Delta t\,sans(v)) sans(f)) (sans(m))=(e^(Delta t sans(v)) sans(f)) (sans(m))=(sans(f) compose phi.alt_(Delta t)^(sans(v)) (sans(m)). $ We can approximate the evolution operator by summing the series up to a given order:
...
Newton\'s equations for the motion of a particle in a plane, subject to a force that depends only on the position in the plane, are a system of second-order differential equations for the rectangular coordinates $(X\,Y)$ of the particle: $ D^2 X (t)= A_x (X (t)\,Y (t))upright(" and ") D^2 Y (t)= A_y (X (t)\,Y (t))\, $ where $A$ is the acceleration of the particle.

These are equivalent to a system of first-order equations for the coordinate path $sigma = chi compose gamma$, where $chi =(sans(t)\,sans(x)\,sans(y)\,sans(v)_x\,sans(v)_y)$ is a coordinate system on the manifold Rh $bb (R)^5$. Then our equations are: $ D (sans(t) compose gamma)= 1 D (sans(x) compose gamma)= sans(v)_x compose gamma D (sans(y) compose gamma)= sans(v)_y compose gamma D (sans(v)_x compose gamma)= A_x (sans(x) compose gamma\,sans(y) compose gamma)D (sans(v)_y compose gamma)= A_y (sans(x) compose gamma\,sans(y) compose gamma)D (sans(t) compose gamma)= 1 $ Construct a vector field on $bb (R)^5$ corresponding to this system of differential equations. Derive the first few terms in the series solution of this problem by exponentiation.
...
The differential of a function is linear in the vector fields. The differential is also a linear operator on functions: if $sans(f)_1$ and $sans(f)_2$ are manifold functions, and if $c$ is a real constant, then $ sans(d) (sans(f)_1 + sans(f)_2)= sans(d f)_1 + sans(d f)_2 $ and $ sans(d) (c sans(f))= c sans(d f) . $ Note that $c$ is not a manifold function.

One-Form Fields

A one-form field is a generalization of this idea; it is something that measures a vector field at each point.

#emph[One-form fields] are linear functions of vector fields that produce real-valued functions on the manifold. A one-form field is linear in vector fields: if $omega$ is a one-form field, $sans(v)$ and $sans(w)$ are vector fields, and $c$ is a manifold function, then $ omega (sans(v) + sans(w))= omega (sans(v))+ omega (sans(w)) $ and $ omega (sans(c v))= sans(c) omega (sans(v)). $

Sums and scalar products of one-form fields on a manifold have the following properties. If $omega$ and $theta$ are one-form fields, and if $sans(f)$ is a real-valued manifold function, then: $ (omega + theta) (sans(v))= omega (sans(v))+ theta (sans(v))\,(sans(f) omega) (sans(v))= sans(f) omega (sans(v)). $

Coordinate-Basis One-Form Fields

Given a coordinate function $chi$, we define the coordinate-basis one-form fields $tilde(sans(X))^i$ by $ tilde(sans(X))^i (sans(v)) (sans(m))= sans(v) (chi^i) (sans(m)) $ or collectively $ tilde(sans(X)) (sans(v)) (sans(m))= sans(v) (chi) (sans(m)). $ With this definition the coordinate-basis one-form fields are dual to the coordinate-basis vector fields in the following sense (see equation 3.15):#footnote[The Kronecker delta $delta_j^i$ is one if $i = j$ and zero otherwise.] $ tilde(sans(X))^i (sans(X)_j) (sans(m))= sans(X)_j(chi^i) (sans(m))= partial_j (chi^i compose chi^(-1)) (chi (sans(m)))= delta_j^i . $ The tuple of basis one-form fields $tilde(X) (sans(v)) (sans(m))$ is an up structure like that of $chi$.

The general one-form field $omega$ is a linear combination of coordinate-basis one-form fields: $ omega (sans(v))=(a compose chi)tilde(sans(X)) (sans(v)) $ with coefficient-function tuple $a (x)$, for $x = chi (sans(m))$. We can write this more simply as $ omega (sans(v))=(a compose chi)tilde(sans(X)) (sans(v))\, $ because everything is evaluated at $sans(m)$.
...
```] $ a_i (x)= omega (tilde(X)_i) (chi^(-1) (x)). $ This follows from the dual relationship (3.41). We can see this as a program:#footnote[The procedure `components->1form-field` is analogous to the procedure `components->vector-field` introduced earlier.]
...
A differential can be expanded in a coordinate basis: $ sans(d f) (sans(v))= sum_i sans(c)_i tilde(sans(X))^i (sans(v)). $ The coefficients $sans(c)_i = sans(d f) (sans(X)_i)= sans(X)_i(sans(f))= partial_i (sans(f) compose chi^(-1))compose chi$ are the partial derivatives of the coordinate representation of $sans(f)$ in the coordinate system of the basis:
...
) The coordinate-basis one-form fields can be used to find the coefficients of vector fields in the corresponding coordinate vector-field basis: $ tilde(sans(X))^i (sans(v))= sans(v) (chi^i)= b^i compose chi $ or collectively, $ tilde(sans(X)) (sans(v))= sans(v) (chi)= b compose chi . $

A coordinate-basis one-form field is often written $sans(d x)^i$. This traditional notation for the coordinate-basis one-form fields is justified by the relation: $ sans(d x)^i = tilde(sans(X))^i = sans(d) (chi^i). $ The `define-coordinates` procedure also makes the basis one-form fields with these traditional names inherited from the coordinates.
...
The coefficients of a differential are (see equation 3.45): $ sans(c)_i = sans(X)_i(sans(f))= sans(d f) (sans(X)_i) $ and partial derivatives of functions commute $ sans(X)_i(sans(X)_j(sans(f)))= sans(X)_j(sans(X)_i(sans(f))). $ As a consequence, the coefficients of a differential are constrained $ sans(X)_i(sans(c)_j)= sans(X)_j(sans(c)_i)\, $ but a one-form field can be constructed with arbitrary coefficient functions. For example: $ sans(x d x) + sans(x d y) $ is not a differential of any function. This is why we started with the basis one-form fields and built the general one-form fields in terms of them.

Coordinate Transformations

Consider a coordinate change from the chart $chi$ to the chart $chi'$. $ tilde(sans(X)) (sans(v))= sans(v) (chi)= sans(v) (chi compose (chi')^(-1)compose chi')=(D (chi compose (chi')^(-1))compose chi')sans(v) (chi')=(D (chi compose (chi')^(-1))compose chi')compose tilde(sans(X))'(v)\, $ where the third line follows from the chain rule for vector fields.

One-form fields are independent of coordinates. So, $ omega (v)=(a compose chi)tilde(sans(X)) (v)=(a' compose chi')tilde(sans(X))'(v). $ Eqs. (3.54) and (3.53) require that the coefficients transform under coordinate transformations as follows: $ a (chi (sans(m)))D (chi compose (chi')^(-1)) (chi'(sans(m)))= a'(chi'(sans(m)))\, $ or $ a (chi (sans(m)))= a'(chi'(sans(m))) (D (chi compose (chi')^(-1)) (chi'(sans(m))))^(-1). $

The coefficient tuple $a (x)$ is a down structure compatible for contraction with $b (x)$. Let $sans(v)$ be the vector with coefficient tuple $b (x)$, and $omega$ be the one-form with coefficient tuple $a (x)$. Then, by equation (3.43), $ omega (sans(v))=(a compose chi) (b compose chi). $ As a program:
...
Comparing equation (3.56) with equation (3.23) we see that one-form components and vector components transform oppositely, so that $ a (x)b (x)= a'(x')b'(x')\, $ as expected because $omega (sans(v)) (sans(m))$ is independent of coordinates.
