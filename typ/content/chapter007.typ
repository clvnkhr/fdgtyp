...
#fdg-chapter("Directional Derivatives", numbered: true)[
...
$ cal(D)_(sans(v)) (sans(f))= sans(v) (sans(f)). $
...
$ g (delta)= sans(u) (sans(f)) (sans(m))-(F_delta^(sans(v)) sans(u)) (sans(f)) (sans(m)). $

So we can compute the directional derivative operator using only ordinary derivatives:

$ cal(D)_(sans(v)) sans(u) (sans(f)) (sans(m))= D g (0). $
...
$ cal(D)_(sans(v)) (alpha sans(O) + beta sans(P))= alpha cal(D)_(sans(v)) sans(O) + beta cal(D)_(sans(v)) sans(P)\, $
...
$ cal(D)_(sans(v) + sans(w)) = cal(D)_(sans(v)) + cal(D)_(sans(w)) . $

The directional derivative is homogeneous over the reals in its vector-field argument:

$ cal(D)_(alpha sans(v)) = alpha cal(D)_(sans(v))\, $
...
$ phi.alt_t^(alpha sans(v)) (sans(m))= phi.alt_(alpha t)^(sans(v)) (sans(m)). $

When applied to products of functions, directional derivative operators satisfy Leibniz\'s rule:

$ cal(D)_(sans(v)) (sans(f) sans(g))= sans(f) (cal(D)_(sans(v)) sans(g))+(cal(D)_(sans(v)) sans(f))sans(g) . $

The Leibniz rule is extended to applications of one-form fields to vector fields:

$ cal(D)_(sans(v)) (omega (sans(y)))= omega (cal(D)_(sans(v)) sans(y))+(cal(D)_(sans(v)) omega) (sans(y)). $
...
$ cal(L)_(sans(v)) sans(f) = sans(v) (sans(f)). $
...
$ (cal(L)_(sans(v)) sans(y)) (sans(f)) (sans(m))= D g (0)\, $

where

$ g (delta)= sans(y) (sans(f)) (sans(m))-((phi.alt_delta^(sans(v)))_(*)sans(y)) (sans(f)) (sans(m)). $
...
$ (cal(L)_(sans(v)) sans(y)) (sans(f))=[sans(v)\,sans(y)] (sans(f))= sans(v) (sans(y) (sans(f)))- sans(y) (sans(v) (sans(f)))= sans(v) (sans(y) (sans(f)))- sans(y) (cal(L)_(sans(v)) (sans(f))). $
...
$ cal(L)_(sans(v)) (sans(g) sans(u)) (sans(f))=[sans(v)\,sans(g) sans(u)] (sans(f))= sans(v) (sans(g))sans(u) (sans(f))+ sans(g)[sans(v)\,sans(u)] (sans(f))=(cal(L)_(sans(v)) sans(g))sans(u) (sans(f))+ sans(g) (cal(L)_(sans(v)) sans(u)) (sans(f)). $

== An Alternate View
We can write the vector field

$ sans(y) (sans(f))= sum_i y^i sans(e)_i(sans(f)). $

By the extended product rule (equation 7.17) we get

$ cal(L)_(sans(v)) sans(y) (sans(f))= sum_i(sans(v) (sans(y)^i)sans(e)_i(sans(f))+ sans(y)^i cal(L)_(sans(v)) cal(e)_i(sans(f))). $
...
$ Delta_j^i (sans(v))= tilde(sans(e))^i (cal(L)_(sans(v)) sans(e)_j)= tilde(sans(e))^i ([sans(v)\,sans(e)_j]). $

So the Lie derivative can be written

$ (cal(L)_(sans(v)) sans(y)) (sans(f))= sum_i #scale (x: 240%, y: 240%)[(] sans(v) (sans(y)^i)+ sum_j Delta_j^i (sans(v))sans(y)^j #scale (x: 240%, y: 240%)[)] sans(e)_i(f). $
...
$ tilde(sans(e))^i (cal(L)_(sans(e)_k) sans(e)_j)= tilde(sans(e))^i ([sans(e)_k\,sans(e)_j])= sans(d)_(k j)^i . $

Resolving $sans(v)$ into components and applying the product rule, we get

$ (cal(L)_(sans(v)) sans(y)) (sans(f))= sum_k #scale (x: 120%, y: 120%)[(] sans(v)^k[sans(e)_k\,sans(y)] (sans(f))- sans(y) (sans(v)^k)sans(e)_k) (sans(f))#scale (x: 120%, y: 120%)[)] . $

So $Delta_j^i$ is related to the structure constants by

$ Delta_j^i (sans(v))= tilde(sans(e))^i (cal(L)_(sans(v)) sans(e)_j)= sum_k #scale (x: 120%, y: 120%)[(] sans(v)^k tilde(sans(e))^i ([sans(e)_k\,sans(e)_j])- sans(e)_j(sans(v)^k)tilde(sans(e))^i (sans(e)_k)#scale (x: 120%, y: 120%)[)] = sum_k #scale (x: 120%, y: 120%)[(] sans(v)^k sans(d)_(k j)^i - sans(e)_j(sans(v)^k)delta_k^i #scale (x: 120%, y: 120%)[)] = sum_k sans(v)^k sans(d)_(k j)^i - sans(e)_j(sans(v)^i). $
...
$ (cal(L)_(sans(v)) (omega)) (sans(y)) "≡" sans(v) (omega (sans(y)))- omega (cal(L)_(sans(v)) sans(y)). $
...
$ cal(L)_(sans(v)) omega (sans(w)_0\,dots.c\,sans(w)_(k - 1))= sans(v) (omega (sans(w)_0\,dots.c\,sans(w)_(k - 1)))- sum_(i=0)^(k - 1) omega (sans(w)_0\,dots.c\,cal(L)_(sans(v)) sans(w)_i\,dots.c\,sans(w)_(k - 1)). $
...
$ (cal(L)_(sans(v)) sans(a)) (sans(b))= sans(v) (sans(a) (sans(b)))- sans(a) (cal(L)_(sans(v)) sans(b)). $
...
$ cal(L)_(alpha sans(v) + beta sans(w)) = alpha cal(L)_(sans(v)) + beta cal(L)_(sans(w))\, $

and

$ cal(L)_(sans(v)) (alpha sans(a) + beta sans(b))= alpha cal(L)_(sans(v)) sans(a) + beta cal(L)_(sans(v)) sans(b)\, $
...
$ cal(L)_(sans(v)) (sans(d) omega)= sans(d) (cal(L)_(sans(v)) omega). $
...
$ [cal(L)_(sans(v))\,cal(L)_(sans(w))]= cal(L)_([sans(v)\,sans(w)]) . $
...
$ e^(t cal(L)_(sans(v))) sans(y) = sans(y) + t cal(L)_(sans(v)) sans(y) + t^2 / 2 cal(L)_(sans(v))^2 sans(y) + dots.c = sans(y) + t[sans(v)\,sans(y)]+ t^2 / 2[sans(v)\,[sans(v)\,sans(y)]]+ dots.c . $
...
$ upright("exp") (alpha cal(L)_((sans(x) thin partial\/partial sans(y) - sans(y) thin partial\/partial sans(x))))frac(partial, partial sans(y)) = - sin(a)frac(partial, partial sans(x)) + cos(a)frac(partial, partial sans(y)) . $
...
$ (i_(sans(v)) omega) (sans(v)_1\,dots.c sans(v)_(sans(p) - 1))= omega (sans(v)\,sans(v)_1\,dots.c sans(v)_(sans(p - 1))). $

There is a mundane identity corresponding to the product rule for the Lie derivative of an interior product:

$ cal(L)_(sans(v)) (i_(sans(y)) omega)= i_(cal(L)_(sans(v)) sans(y)) omega + i_(sans(y)) (cal(L)_(sans(v)) omega). $
...
$ cal(L)_(sans(v)) omega = i_(sans(v)) (sans(d) omega)+ sans(d) (i_(sans(v)) omega). $
...
$ F_delta^(sans(v)) sans(u) (sans(f)) (sans(m))= sum_(i\,j) (A_j^i (delta) (sans(u)^j compose phi.alt_(- delta)^(sans(v)))sans(e)_i(sans(f))) (sans(m)) $
...
$ nabla_(sans(v)) sans(u) (sans(f)) (sans(m))= D g (0)\, $
...
$ g (delta)= sum_i #scale (x: 300%, y: 300%)[(] sans(u)^i sans(e)_i(sans(f))- sum_j A_j^i (delta) (sans(u)^j compose phi.alt_(- delta)^(sans(v)))sans(e)_i(sans(f))#scale (x: 300%, y: 300%)[)] (sans(m)). $

By the product rule for derivatives,

$ D g (delta)= sum_(i j) #scale (x: 120%, y: 120%)[(] A_j^i (delta) ((sans(v) (sans(u)^j))compose phi.alt_(- delta)^(sans(v)))sans(e)_i(sans(f))- D A_j^i (delta) (sans(u)^j compose phi.alt_(- delta)^(sans(v)))sans(e)_i(sans(f))#scale (x: 120%, y: 120%)[)] (sans(m)). $

So, since $A_j^i (0) (sans(m))$ is the identity multiplier, and $phi.alt_0^(sans(v))$ is the identity function,

$ D g (0)= sum_i #scale (x: 300%, y: 300%)[(] sans(v) (sans(u)^i) (sans(m))sans(e)_i(sans(f))- sum_j D A_j^i (0)sans(u)^j (sans(m))sans(e)_i(sans(f))#scale (x: 300%, y: 300%)[)] thin (sans(m)). $
...
$ frac(d, d delta) (B (sans(v)) (delta))= frac(d, d delta) (B (xi sans(v)) (delta\/xi)). $

Using the chain rule

$ D (B (sans(v))) (delta)= 1 / xi D (B (xi sans(v))) (delta / xi)\, $

so, for $delta = 0$,

$ xi D (B (sans(v))) (0)= D (B (xi sans(v))) (0). $
...
$ D g (0)= sum_i (sans(v) (sans(u)^i) (sans(m)) + sum_j pi.alt_j^i (sans(v)) (sans(m)) sans(u)^j (sans(m))) sans(e)_i(sans(f)) (sans(m)). $

Thus the covariant derivative is

$ nabla_(sans(v)) sans(u) (sans(f))= sum_i (sans(v) (sans(u)^i) + sum_j pi.alt_j^i (sans(v)) sans(u)^j) sans(e)_i(sans(f)). $
...
$ nabla_(sans(g) sans(v)) sans(u) (sans(f))= sans(g) nabla_(sans(v)) sans(u) (sans(f))\, $

consistent with the fact that the Cartan forms $pi.alt_j^i$ share the same property.

Additionally, we can extend the product rule, for any manifold function $sans(g)$ and any vector field $sans(u)$:

$ nabla_(sans(v)) (sans(g) sans(u)) (sans(f))= sum_i (sans(v) (sans(g u)^i) + sum_j pi.alt_j^i (sans(v)) sans(g u)^j) sans(e)_i(sans(f))= sum_i sans(v) (sans(g))sans(u)^i sans(e)_i(sans(f))+ sans(g) nabla_(sans(v)) (sans(u)) (sans(f))=(nabla_(sans(v)) sans(g))sans(u) (sans(f))+ sans(g) nabla_(sans(v)) (sans(u)) (sans(f)). $

== An Alternate View
As we did with the Lie derivative (equations 7.18-7.21), we can write the vector field

$ sans(u) (sans(f)) (sans(m))= sum_i sans(u)^i (sans(m))sans(e)_i(sans(f)) (sans(m)). $

By the extended product rule, equation (7.51), we get:

$ nabla_(sans(v)) sans(u) (sans(f))= sum_i(sans(v) (sans(u)^i)sans(e)_i(sans(f))+ sans(u)^i nabla_(sans(v)) sans(e)_i(sans(f))). $
...
$ pi.alt_j^i (sans(v))= tilde(sans(e))^i (nabla_(sans(v)) sans(e)_j). $
...
$ pi.alt_j^i (sans(v)) (sans(m))= sum_k Gamma_(j k)^i (sans(m))tilde(sans(e))^k (sans(v)) (sans(m)). $
...
$ nabla_(sans(v)) (tau (sans(w)))= sans(v) (sum_j tau_j sans(w)^j) = sum_j(sans(v) (tau_j)sans(w)^j + tau_j sans(v) (sans(w)^j))= sum_j (sans(v) (tau_j) sans(w)^j + tau_j (tilde(sans(e))^j (nabla_(sans(v)) sans(w)) - sum_k pi.alt_k^j (sans(v)) sans(w)^k)) = sum_j (sans(v) (tau_j) sans(w)^j - tau_j sum_k pi.alt_k^j (sans(v)) sans(w)^k) + tau (nabla_(sans(v)) sans(w))= sum_j (sans(v) (tau_j) tilde(sans(e))^j - tau_j sum_k pi.alt_k^j (sans(v)) tilde(sans(e))^k) (sans(w))+ tau (nabla_(sans(v)) sans(w)). $

So if we define the covariant derivative of a one-form field to be

$ nabla_(sans(v)) (tau)= sum_k (sans(v) (tau_k) - sum_j tau_j pi.alt_k^j (sans(v))) tilde(sans(e))^k\, $

then the generalized product rule holds:

$ nabla_(sans(v)) (tau (sans(u)))=(nabla_(sans(v)) tau) (sans(u))+ tau (nabla_(sans(v)) sans(u)). $
...
$ sans(e) (sans(f))= sans(e)'(sans(f))sans(J) . $
...
$ nabla_(sans(v)) sans(u) (sans(f))= sum_i sans(e)_i(sans(f)) (sans(v) (sans(u)^i) + sum_j pi.alt_j^i (sans(v)) upright(u)^j) = sum_(i j k) sans(e)'_i(sans(f))sans(J)_j^i (sans(v) ((sans(J)^(-1))_k^j (sans(u)')^k) + sum_l pi.alt_k^j (sans(v)) (sans(J)^(-1))_l^k (sans(u)')^l) = sum_i sans(e)'_i(sans(f)) (sans(v) ((sans(u)')^i) + sum_(j k) sans(J)_j^i sans(v) ((sans(J)^(-1))_k^j) (sans(u)')^k + sum_(j k l) sans(J)_j^i pi.alt_k^j (sans(v)) (sans(J)^(-1))_l^k (sans(u)')^l) = sum_i sans(e)'_i(sans(f)) (sans(v) ((sans(u)')^i) + sum_j (pi.alt')_j^i (sans(v)) (sans(u)')^j) . $
...
$ pi.alt'(sans(v))= sans(J v) (sans(J)^(-1))+ sans(J) pi.alt (sans(v))sans(J)^(-1) . $
...
$ sans(e)_i(sans(f))= sum_j sans(e)'_j(sans(f))sans(J)_i^j . $

Let $sans(K) = sans(J)^(-1)$, so $sum_j sans(K)_j^i (sans(m))sans(J)_k^j (sans(m))= delta_k^i$. Then

$ pi.alt'_l^i (sans(v))= sum_j sans(J)_j^i sans(v) (sans(K)_l^j)+ sum_(j k) sans(J)_j^i pi.alt_k^j (sans(v))sans(K)_l^k . $
...
$ F_delta^(sans(w)_gamma) sans(u)_gamma(sans(f)) (t)= sum_(i\,j) A_j^i (delta)u^j (t - delta)sans(e)_i^gamma (sans(f)) (t). $] So a vector field $sans(u)_gamma$ is parallel-transported to itself if and only if $sans(u)_gamma = F_delta^(sans(w)_gamma) sans(u)_gamma$. Restricted to a path, the equation analogous to equation (7.40) is

$ g (delta)= sum_i (u^i (t) - sum_j A_j^i (delta) u^j (t - delta)) sans(e)_i^gamma (sans(f)) (sans(t))\, $
...
$ 0 = sum_i (D u^i (t) + sum_j^gamma pi.alt_j^i (sans(w)_gamma) (t) u^j (t)) sans(e)_i^gamma (sans(f)) (sans(t)). $

But this implies that

$ 0 = D u^i (t)+ sum_j^gamma pi.alt_j^i (sans(w)_gamma) (sans(t))u^j (t)\, $
...
$ nabla_(sans(v))^gamma sans(u)_gamma(sans(f)) (sans(n))= sum_i (sans(v) (u^i) (sans(n)) + sum_j^gamma pi.alt_j^i (d gamma (sans(v))) (sans(n)) u^j (sans(n))) sans(e)_i^gamma (sans(f)) (sans(n))\, $
...
$ nabla_(sans(v))^gamma sans(u)_gamma(sans(f)) (sans(n))= nabla_(sans(w)) sans(u) (sans(f)) (gamma (sans(n))). $
...
$ nabla_(partial\/partial sans(t))^gamma sans(u)_gamma = 0 . $
...
$ D u^i (t)+ sum_j^gamma pi.alt_j^i (d gamma (partial\/partial t)) (sans(t))u^j (t)= 0 . $

Expressing the Cartan forms in terms of the Christoffel coefficients we obtain

$ D u^i (t)+ sum_(j\,k) Gamma_(j k)^i (gamma (sans(t)))D sigma^k (t)u^j (t)= 0 $
...
$ D u^0 (tau)= sin(alpha (tau))cos(alpha (tau))D beta (tau)u^1 (tau)\, $

$ D u^1 (tau)= - frac(cos(alpha (tau)), sin(alpha (tau))) (D beta (tau)u^0 (tau)+ D alpha (tau)u^1 (tau)). $

These equations describe the transport on a sphere, but more generally they look like

$ D u (tau)= f (sigma (tau)\,D sigma (tau))u (tau)\, $
...
$ nabla_(partial\/partial sans(t))^gamma d gamma (partial\/partial sans(t))= 0 . $
