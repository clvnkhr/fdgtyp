...
#fdg-chapter("Basis Fields", numbered: true)[
...
$ sans(v) (sans(f)) (sans(m))= sans(e) (sans(f)) (sans(m))sans(b) (sans(m))= sum_i sans(e)_i(sans(f)) (sans(m))sans(b)^i (sans(m))\, $
...
$ sans(e)_j(sans(f))= sum_k sans(X)_k(sans(f))sans(c)_j^k\, $
...
$ tilde(sans(e))^i (sans(e)_j) = sum_l sans(d)_l^i tilde(sans(X))^l (sans(e)_j) = sum_l sans(d)_l^i sans(e)_j (chi^l) = sum_l sans(d)_l^i sum_k sans(X)_k (chi^l) sans(c)_j^k = sum_(k l) sans(d)_l^i delta_k^l sans(c)_j^k = sum_k sans(d)_k^i sans(c)_j^k . $

Applying this at $sans(m)$ we get

$ tilde(sans(e))^i (sans(e)_j) (sans(m))= delta_j^i = sum_k sans(d)_k^i (sans(m))sans(c)_j^k (sans(m)). $
...
$ tilde(sans(e))^j (sans(v))= sans(b)^j . $
...
$ sans(v) (sans(f))= sum_i sans(e)_i(sans(f))sans(b)^i = sum_i sans(e')_j(sans(f))sans(b')^j . $

The coefficients $sans(b')$ can be obtained from $sans(v)$ by applying the dual basis

$ sans(b')^j = sans(tilde(e)')^j (sans(v))= sum_i sans(tilde(e)')^j (sans(e)_i)sans(b)^i . $

Let

$ sans(J)_i^j = sans(tilde(e)')^j (sans(e)_i)\, $

then

$ sans(b')^j = sum_i sans(J)_i^j sans(b)^i\, $
...
$ sans(e') (sans(f))= sans(e) (sans(f))sans(K) $

then

$ sans(K) sans(J) = sans(I) = sans(J) sans(K) $

where $sans(I)$ is a manifold function that returns the multiplicative identity.

The dual basis transforms oppositely. Let

$ bold(omega) = sum_i sans(a)_i tilde(sans(e))^(' i) . $<eq:op-transform>
...
$ sans(a)_i = bold(omega) (sans(e)_i)= sum_j sans(a)'_j tilde(sans(e))^(' j) (sans(e)_i)= sum_j sans(a)'_j sans(J)_i^j $

or, in tuple arithmetic,

$ sans(a) = sans(a)' sans(J) . $

Because of equation (4.18) we can deduce

$ tilde(sans(e)) = sans(K) tilde(sans(e))' . $
...
$ R_x (epsilon.alt)M (theta\,phi.alt\,psi)= M (theta + a epsilon.alt\,phi.alt + b epsilon.alt\,psi + c epsilon.alt). $

Linear equations for $(a\,b\,c)$ can be found by taking the derivative of this equation with respect to $epsilon.alt$. We find

$ 0 = c cos theta + b\, $

$ 0 = a sin phi.alt - c cos phi.alt sin theta\, $

$ 1 = c sin phi.alt sin theta + a cos phi.alt\, $

with the solution

$ a = cos phi.alt\, $

$ b = - frac(sin phi.alt cos theta, sin theta)\, $

$ c = frac(sin phi.alt, sin theta) . $
...
$ sans(e)_x = a frac(partial, partial theta) + b frac(partial, partial phi.alt) + c frac(partial, partial psi) = cos phi.alt frac(partial, partial theta) - frac(sin phi.alt cos theta, sin theta) frac(partial, partial phi.alt) + frac(sin phi.alt, sin theta) frac(partial, partial psi) . $

Similarly, vector fields for the incremental y and z rotations are

$ sans(e)_y = frac(cos phi.alt cos theta, sin theta) frac(partial, partial phi.alt) + sin phi.alt frac(partial, partial theta) - frac(cos phi.alt, sin theta) frac(partial, partial psi) $

$ sans(e)_z = frac(partial, partial phi.alt) . $

== Commutators
The commutator of two vector fields is defined as

$ [sans(v)\,sans(w)] (sans(f))= sans(v) (sans(w) (sans(f)))- sans(w) (sans(v) (sans(f))). $

In the special case that the two vector fields are coordinate basis fields, the commutator is zero:

$ [sans(X)_i \, sans(X)_j] (sans(f))= sans(X)_i (sans(X)_j (sans(f))) - sans(X)_j (sans(X)_i (sans(f))) = partial_i partial_j (sans(f) compose chi^(-1)) compose chi - partial_j partial_i (sans(f) compose chi^(-1)) compose chi = 0\, $
...
$ [sans(u)\,sans(v)] (sans(f))= sans(u) (sans(v) (sans(f)))- sans(v) (sans(u) (sans(f)))= sans(u) (sum_i sans(X)_i (sans(f)) sans(c)^i) - sans(v) (sum_j sans(X)_j (sans(f)) sans(b)^j) = sum_j sans(X)_j (sum_i sans(X)_i (sans(f)) sans(c)^i) sans(b)^j - sum_i sans(X)_i (sum_j sans(X)_j (sans(f)) sans(b)^j) sans(c)^i = sum_(i j) [sans(X)_j \, sans(X)_i] (sans(f))sans(c)^i sans(med b)^j + sum_i sans(X)_i(sans(f))sum_j (sans(X)_j (sans(c)^i) sans(b)^j - sans(X)_j (sans(med b)^i) sans(c)^j) = sum_i sans(X)_i(sans(f))sans(a)^i\, $

where the coefficient function $sans(a)$ of the commutator vector field is

$ sans(a)^i = sum_j (sans(X)_j (sans(c)^i) sans(b)^j - sans(X)_j (sans(b)^i) sans(c)^j) = sans(u) (sans(c)^i) - sans(v) (sans(b)^i) . $
...
$ [sans(e)_i\,sans(e)_j] (sans(f))= sum_k sans(d)_(i j)^k sans(e)_k(sans(f))\, $
...
$ sans(d)_(i j)^k = tilde(sans(e))^k ([sans(e)_i \, sans(e)_j]) . $

The commutator $[sans(u)\,sans(v)]$ with respect to a non-coordinate basis $sans(e)_i$ is

$ [sans(u)\,sans(v)] (sans(f))= sum_k sans(e)_k (sans(f)) (sans(u) (sans(c)^k) - sans(v) (sans(b)^k) + sum_(i j) sans(c)^i sans(b)^j sans(d)_(j i)^k) $
...
$ [sans(J)_x \, sans(J)_y] = - sans(J)_z [sans(J)_y \, sans(J)_z] = - sans(J)_x [sans(J)_z \, sans(J)_x] = - sans(J)_y $
...
$ (e^(t sans(v))) (sans(m)) = (sans(f) compose phi.alt_t^(sans(v))) (sans(m)) . $

Iterating this equation, we find

$ (e^(s sans(w)) e^(t sans(v))) (sans(m)) = (sans(f) compose phi.alt_t^(sans(v)) compose phi.alt_s^(sans(w))) (sans(m)) . $
...
$ (e^(epsilon.alt sans(v)) e^(epsilon.alt sans(w)) e^(- epsilon.alt sans(v)) e^(- epsilon.alt sans(w)) sans(f)) (sans(m)) . $
...
$ (e^(epsilon.alt^2[sans(v)\,sans(w)]) sans(f)) (sans(m)) $
...
$ sans(m) = phi.alt_1^(x sans(e)) (sans(0))= chi^(-1) (x)\, $
