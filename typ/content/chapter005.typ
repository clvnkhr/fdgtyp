...
#fdg-chapter("Integration", numbered: true)[
...
$ integral_a^b f = integral_a^b f (x)d x = lim_(Delta x_i arrow.r 0) sum_i f (x_i)Delta x_i $

If we change variables $(x = g (y))$, then the form of the integral changes:

$ integral_a^b f = integral_a^b f (x)d x = integral_(g^(-1) (a))^(g^(-1) (b)) f (g (y))D g (y)d y = integral_(g^(-1) (a))^(g^(-1) (b)) (f compose g)D g $
...
$ I = integral_a^b omega (partial\/partial sans(x))compose chi^(-1) $
...
$ integral_(a')^(b') bold(omega) (partial \/ partial upright(x)') compose chi^(' - 1) = integral_(a')^(b') bold(omega) (partial \/ partial upright(x) (D (chi compose chi^(' - 1)) compose chi')) compose chi^(' - 1) = integral_(a')^(b') (bold(omega) (partial \/ partial upright(x)) D (chi compose chi^(' - 1)) compose chi') compose chi^(' - 1) = integral_(a')^(b') (bold(omega) (partial \/ partial upright(x)) compose chi^(' - 1)) D (chi compose chi^(' - 1)) = integral_a^b (((bold(omega) (partial \/ partial upright(x)) compose chi^(-1)) D (chi compose chi^(' - 1))) compose g) D g = integral_a^b bold(omega) (partial\/partial upright(x))compose chi^(-1)\, $
...
$ integral_(sans(U)) sans(f) = integral_(g^(-1) (sans(U))) (sans(f) compose g)det(D g). $
...
$ integral_(chi (sans(U))) omega (sans(X)_0\,sans(X)_1\,dots)compose chi^(-1) . $

Under coordinate transformations $g = chi compose chi'^(-1)$, the integral becomes

$ integral_(chi'(sans(U))) bold(omega) (sans(X)_0\,sans(X)_1\,dots)compose chi^(' - 1) det(D g) . $

Using the change-of-basis formula, equation (3.19):

$ sans(X) (sans(f))= sans(X)'(sans(f)) (D (chi' compose chi^(-1)))compose chi = sans(X)'(sans(f)) (D (g^(-1))compose chi . $

If we let $M =(D (g^(-1)))compose chi$ then

$ (omega (sans(X)_0\,sans(X)_1\,dots)compose chi^(' - 1))det(D g)=(omega (sans(X)' M_0\,sans(X)' M_1\,dots)compose chi^(' - 1))det(D g)=(omega (sans(X)'_0\,sans(X)'_1\,dots)compose chi^(' - 1))alpha (M_0\,M_1\,dots)det(D g)\, $
...
$ I' = integral_(chi'(sans(U))) omega (sans(X)'_0\,sans(X)'_1\,dots)compose chi^(' - 1) . $
...
$ (omega and tau) (sans(v)\,sans(w))= omega (sans(v))tau (sans(w))- omega (sans(w))tau (sans(v)). $
...
$ omega and tau = frac((k + l)!, k ! l !) upright("Alt") (omega "⊗" tau) $

where, if η is a function on $m$ vectors,

$ upright("Alt") (eta) (sans(v)_0\,dots\,sans(v)_(m - 1))= frac(1, m !) sum_(sigma epsilon.alt upright("Perm") (m)) upright("Parity") (sigma)eta (sans(v)_(sigma (0))\,dots\,sans(v)_(sigma (m - 1)))\, $

and where

$ omega "⊗" tau (sans(v)_0\,dots\,sans(v)_(k - 1)\,sans(v)_k\,dots\,sans(v)_(k + l - 1))= omega (sans(v)_0\,dots\,sans(v)_(k - 1))tau (sans(v)_k\,dots\,sans(v)_(k + l - 1)). $
...
$ (sans(d) sans(x) and sans(d) sans(y) and dots) (partial\/partial sans(x))\,partial\/partial sans(y)\,dots)= 1 . $

This is true independent of the coordinate system.

Equation (5.17) gives us

$ integral_(sans(U)) sans(d) sans(x) and sans(d) sans(y) and dots.c = upright("Volume") (sans(U)) $
...
$ sans(A) (sans(u)\,sans(v)) (sans(m))= sans(u)^0(sans(m))sans(v)^1 - sans(v)^0(sans(m))sans(u)^1(sans(m)). $
...
$ omega = sum_(i_0\,dots.c\,i_(k - 1))^n omega_(i_0\,dots.c\,i_(k - 1)) sans(d) sans(x)^(i_0) and dots.c and sans(d) sans(x)^(i_(k - 1)) . $

The antisymmetry of the wedge product implies that

$ omega_(i_(sigma (0))\,dots.c\,i_(sigma (k - 1))) = upright("Parity") (sigma)omega_(i_0\,dots.c\,i_(k - 1))\, $
...
$ integral_(partial sans(M)) omega = integral_(sans(M)) sans(d) omega . $
...
$ sans(d) omega (sans(v)_1\,sans(v)_2)= sans(v)_1(omega (sans(v)_2))- sans(v)_2(omega (sans(v)_1))- omega ([sans(v)_1\,sans(v)_2]). $
...
$ sans(d) omega (sans(v)_0\,dots.c\,sans(v)_k)= sum_(i=0)^k {((- 1)^i sans(v)_i (omega (sans(v)_0 \, dots.c \, sans(v)_(i - 1) \, sans(v)_(i + 1) \, dots.c \, sans(v)_k)) + sum_(j=i + 1)^k (- 1)^(i + j) omega (sans(v)_i \, sans(v)_j] \, sans(v)_0 \, dots.c \, sans(v)_(i - 1) \, sans(v)_(i + 1) \, dots.c \, sans(v)_(j - 1) \, sans(v)_(j + 1) \, dots.c \, sans(v)_k))} . $
...
$ omega = sum_(i_0=0\,dots.c\,i_(k - 1) = 0)^(n - 1) sans(a)_(i_0\,dots.c\,i_(k - 1)) sans(d) sans(x)^(i_0) and dots.c and sans(d) sans(x)^(i_(k - 1)) $

then the exterior derivative can be expressed as

$ sans(d) omega = sum_(i_0=0\,dots.c\,i_(k - 1) = 0)^(n - 1) sans(d) sans(a)_(i_0\,dots.c\,i_(k - 1)) sans(d) sans(x)^(i_0) and dots.c and sans(d) sans(x)^(i_(k - 1)) . $
...
$ omega = a sans(d) sans(y) and sans(d) sans(z) + b sans(d) sans(z) and sans(d) sans(x) + c sans(d) sans(x) and sans(d) sans(y)\, $
...
$ sans(d) (omega and tau)= sans(d) omega and tau +(- 1)^k omega and sans(d) tau\, $
...
$ sans(d)^2 sans(f) (sans(u)\,sans(v))= sans(d) (sans(d) sans(f)) (sans(u)\,sans(v))= sans(u) (sans(d) sans(f) (sans(v)))- sans(v) (sans(d) sans(f) (sans(u)))- sans(d) sans(f) ([sans(u)\,sans(v)])= sans(u) (sans(v) (sans(f)))- sans(v) (sans(u) (sans(f)))-[sans(u)\,sans(v)] (sans(f))= 0 $
...
$ integral_(partial sans(P)) omega = integral_(sans(P)) sans(d) omega . $
...
$ integral_(chi (sans(P))) sans(d) omega (partial\/partial sans(x)\,partial\/partial sans(y))compose chi^(-1) = integral_(x_(m i n))^(x_(m a x)) integral_(y_(m i n))^(y_(m a x)) (partial\/partial sans(x) (omega (partial\/partial sans(y)))- partial\/partial sans(y) (omega (partial\/partial sans(x))))compose chi^(-1) . $
...
$ integral_(partial M) ((alpha compose chi)sans(d) sans(x) +(beta compose chi)sans(d) sans(y))= integral_M ((partial_0 beta - partial_1 alpha)compose chi)sans(d) sans(x) and sans(d) sans(y) . $
...
$ integral_M upright(div) (sans(w))d V = integral_(partial M) sans(w) dot.op sans(n) d A $
...
$ sans(w) = sans(a) frac(partial, partial sans(x)) + sans(b) frac(partial, partial sans(y)) + sans(c) frac(partial, partial sans(z)) . $
...
$ sans(a) thin sans(d) sans(y) and sans(d) sans(z) + sans(b) thin sans(d) sans(z) and sans(d) sans(x) + sans(c) thin sans(d) sans(x) and sans(d) sans(y)\, $
...
$ (frac(partial, partial sans(x)) sans(a) + frac(partial, partial sans(y)) sans(b) + frac(partial, partial sans(z)) sans(c))med sans(d) sans(x) and sans(d) sans(y) and sans(d) sans(z) . $
