...
$ sans(T) (sans(f) sans(u) + sans(g) sans(v)\,bold(omega)) &= sans(f) thin sans(T) (sans(u)\,bold(omega))+ sans(g) sans(T) (sans(v)\,bold(omega)) $ <F.1>

$ sans(T) (sans(u)\,sans(f) bold(omega) + sans(g) bold(theta)) &= sans(f) thin sans(T) (sans(u)\,bold(omega))+ sans(g) sans(T) (sans(u)\,bold(theta))\, $ <F.2>
...
$ sans(T)_j^i = sans(T) (e_j\,tilde(e)^i)\, $ <F.3>
...
$ sans(e)_i (sans(f))= sum_j sans(e)' (sans(f))sans(J)_i^j\, $ <F.4>

Where $sans(J)$ typically depends on place. The corresponding dual basis transforms as

$ tilde(sans(e))^i (sans(v))= sum_j sans(K)_j^i tilde(sans(e))^(' j) (sans(v))\, $ <F.5>
...
$ sans(T)_j^i = sum_(k l) sans(K)_k^i sans(T)_l^(' k) sans(J)_j^l\, $ <F.6>

or

$ sans(T)_j^(' i) = sum_(k l) sans(J)_k^i sans(T)_l^k sans(K)_j^l . $ <F.7>
...
;; (up 0 0 0 0)
fdg-code-source-end */
#fdg-cljs-code-block("appendix_c/001")
...
$ sans(R)_(j k l)^i = sans(R) (tilde(sans(e))^i\,sans(e)_j\,sans(e)_k\,sans(e)_l)\, $ <F.8>

then

$ sans(R)_(j k l)^i = sum_(m n p q) sans(K)_m^i sans(R)_(n p q)^(' m) sans(J)_j^n sans(J)_k^p sans(J)_l^q\, $ <F.9>

or

$ sans(R)_(j k l)^(' i) = sum_(m n p q) sans(J)_m^i sans(R)_(n p q)^m sans(K)_j^n sans(K)_k^p sans(K)_l^q . $ <F.10>
...
;; (0 0 0 0)
fdg-code-source-end */
#fdg-cljs-code-block("appendix_c/002")
...
;; (up 0 0 0)
fdg-code-source-end */
#fdg-cljs-code-block("appendix_c/003")
...
$ sans(F) (bold(omega)\,sans(u)\,sans(v))= bold(omega) (nabla_(sans(u)) sans(v))\, $ <F.11>
...
;; 0
fdg-code-source-end */
#fdg-cljs-code-block("appendix_c/004")
...
;; (0 0 MESS)
fdg-code-source-end */
#fdg-cljs-code-block("appendix_c/005")
