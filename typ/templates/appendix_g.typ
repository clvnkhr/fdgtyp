...
Some scmutils calculations are mathematically modest but create very large intermediate expressions in Emmy. The runner does not replace these calculations with floating-point samples. Instead, the converter rewrites the affected listing into smaller exact identities whose conjunction proves the original result. Every residual is simplified symbolically and passed to #raw(lang:"clojure", "verified-zero"); a nonzero component fails the block.

Three listings currently use this treatment:
