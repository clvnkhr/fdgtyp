...
that workspace replaces `build/current`. The source checkout is therefore never
changed by a build; its generated files are inputs to a snapshot, not mutable
build state. Only the latest successful build is retained.
...
final status. To choose a readable identifier, use
`make <target> FDG_RUN_ID=my-run`. A failed build is discarded and leaves the
previous `build/current` intact.
